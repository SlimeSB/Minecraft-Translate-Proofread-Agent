...
指数退避 5 次，delay = min(2^retry, 60)s。可重试：429/connection/timeout/reset/refused/eof/500-504。不可重试：401/403/400/404。日志：`logs/latest.log`（当前）+ `logs/latest.YYYYMMDD-HHMMSS.log`（归档）。
