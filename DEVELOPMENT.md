...
> 源码总计约 5578 行 Python（39 个源文件），测试约 2819 行（17 个文件，279 个用例）。详见末尾 [已知技术债](#已知技术债)。
...
run.py                              # CLI 入口（268行），参数解析，模式分发
  ...
  └─ 构建 LLM callable             # create_openai_llm_call()
       │
       ▼
ReviewPipeline                      # 薄编排器 (111 行)，6 阶段纯函数调用
...
| 文件 | 职责 |
|------|------|
| `src/llm/client.py` (124 行) | `create_openai_llm_call()` — OpenAI 兼容客户端 + 指数退避重试 + 日志滚动 |
...
| `src/llm/bridge.py` (360 行) | `LLMBridge` 类 — `review_batch()`（异步批处理审校）、`filter_verdicts()`（Phase 4 过滤）。`parse_review_response()` — 响应解析（4层容错）。`interactive_entry_review()` — 交互模式。 |
| `src/dictionary/external.py` (151 行) | `ExternalDictStore` — 按需 SQLite 查询社区词典，`lookup()` 按英文单词匹配历史翻译并注入 LLM 提示词。 |
...
> 最近重构已修复：
> - ~~`print()`/`sys.stderr` 混用~~ → `src/logging.py` 统一日志
> - ~~`is_likely_code_or_proper_noun()` 重复~~ → `src/tools/code_detection.py`
> - ~~`run_pr_aligner()` 179 行 God function~~ → 6 个子函数
> - ~~`PipelineDB` 无 context manager~~ → 所有 Phase 改用 `with` 语法
> - ~~`VERDICT_PRIORITY` 重复定义~~ → 从 `models.py` 导入
> - ~~`build_glossary()` 117 行~~ → 23 行，拆为 `_collect_zh_translations` + `_dedup_zh_conflicts`
> - ~~硬编码值散落~~ → `fts_recall_multiplier/min`, `fuzzy_trigger_patterns`, `review_batch_size` 等迁入 `review_config.json`
> - ~~`_batch_process._process` / `filter_verdicts._run_all._process` 重复~~ → 提取 `_llm_call_with_retry` 共享辅助
> - ~~`lemma_cache.py:42` `except Exception`~~ → 改为 `except ImportError`
> - ~~`external.py:60` silent `pass`~~ → 改为 `warn()`
> - ~~3 处 Silent `except: pass`~~ → 全部改为 `warn()` 日志
> - ~~`apply_cache_merge()` 和 `apply_llm_merge()` 约 70% 重复~~ → 提取 `_apply_merge_map` 共享
> - ~~`_check_placeholder_integrity()` 3 次重复比较~~ → 提取 `_compare_placeholder_lists` 共享辅助
> - ~~`import time as _time` 在 `main()` 内~~ → 已移到模块顶部
> - ~~`review_batch()` vs `review_untranslated()` 约 80% 重复~~ → 提取 `_batch_process` 共享异步批处理
...
- **pyright 配置了大量 `=false`** (`pyrightconfig.json`): `reportArgumentType`, `reportAssignmentType`, `reportReturnType`, `reportAttributeAccessIssue`, `reportOptionalMemberAccess` 等 14 项核心检查关闭。TypedDict 约束形同虚设，全局大量 `dict[str, Any]` 绕过类型系统。
- **LLM 模块测试覆盖到达，但质量有限**: `bridge.py`(360行)、`prompts.py`(378行)、`client.py`(124行) 共 862 行，新增 `test_llm_bridge.py`(362行)、`test_llm_client.py`(126行)、`test_llm_prompts.py`(329行) 覆盖，但多为 mock 测试，未覆盖真实 API 交互。

### Medium — 代码结构

- **`TerminologyBuilder` 类承担过多职责**: 术语提取 + 词形归并 + 表构建 + LLM校验 + 一致性检查，约 479 行 10 方法。
- **FTS recall 上限仍留硬编码**: `fuzzy_search.py:112` 配置化但不完善，依赖 `fts_recall_multiplier` 和 `fts_recall_min` 配合调节。
...
| `src/pipeline/phase1_alignment.py` | 110 | 仅在集成测试间接触及 |
| `src/pipeline/phase2_terminology.py` | 33 | 仅在集成测试间接触及 |
| `src/pipeline/phase3a_format.py` | 35 | 仅在集成测试间接触及 |
| `src/pipeline/phase3b_fuzzy.py` | 37 | 仅在集成测试间接触及 |
| `src/pipeline/phase3c_review.py` | 131 | 仅在集成测试间接触及 |
| `src/pipeline/phase4_filter.py` | 101 | 有独立测试 (45 行) |
| `src/pipeline/phase5_report.py` | 194 | 无独立测试 |
| `src/cli.py` | 106 | 无独立测试 |
| `src/tools/pr/__init__.py` | 259 | 无独立测试（仅 `_lang`/`_guideme` 有） |
