...
```
run.py                              # CLI 入口（269行），参数解析，模式分发
  ├─ src/cli.py                     # CLI 工具函数（dotenv加载, API健康检查, UTF-8配置, safe_print）
  │
  ...
  └─ 构建 LLM callable             # create_openai_llm_call()
       │
       ▼
ReviewPipeline                      # 薄编排器 (112 行)，6 阶段纯函数调用
...
### 4.5. `src/tools/term_validation.py` — 共享术语验证

合并自 `terminology_extract.py`、`external.py`、`terminology_builder.py`、`lemma_merge.py` 四处独立逻辑：

```python
STOP_WORDS: set[str]      # 从 config term_blacklist 懒加载、缓存一次
is_valid_term(term) -> bool  # 统一术语有效性检查（长度/数字/停用词）
```
...
Console 输出使用 `cli.safe_print()` 处理 Windows GBK 终端兼容。
...
指数退避 5 次，基数为 5s：5, 10, 20, 40, 60s（上限 60s）。可重试：429/connection/timeout/reset/refused/eof/500-504。不可重试：401/403/400/404。日志：`logs/latest.log`（当前）+ `logs/latest.YYYY-MM-DD-HHMMSS.log`（归档）。每次调用带 `[label#N] [id=uuid8]` 标签，便于异步日志中对齐 request/response。`label` 参数在 `create_openai_llm_call()` 中设置（如 `"Review"`、`"Filter"`）。
