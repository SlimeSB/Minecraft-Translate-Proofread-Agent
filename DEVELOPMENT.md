...
| `src/llm/prompts.py` (280+ 行) | `build_review_prompt()`、`build_filter_prompt()`、`classify_entries()`、`filter_for_llm()`、`merge_multipart_entries()` 等。所有提示词构建与条目筛选逻辑。外部词典提示注入。 |
...
| `src/dictionary/external.py` (130+ 行) | `ExternalDictStore` — 按需 SQLite 查询社区词典，`lookup()` 按英文单词匹配历史翻译并注入 LLM 提示词。 |
