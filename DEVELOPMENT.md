...
> 源码总计约 4600 行 Python（30 个源文件），测试约 1587 行（15 个文件，184 个用例）。详见末尾 [已知技术债](#已知技术债)。

## 架构概览

```
run.py                              # CLI 入口（268行），参数解析，模式分发
  ...
  └─ 构建 LLM callable             # create_openai_llm_call()
       │
       ▼
ReviewPipeline                      # 薄编排器 (98 行)，6 阶段纯函数调用
...
| 文件 | 职责 |
|------|------|
| `src/llm/client.py` (105 行) | `create_openai_llm_call()` — OpenAI 兼容客户端 + 指数退避重试 + 日志滚动 |
| `src/llm/prompts.py` (334 行) | `build_review_prompt()`、`build_filter_prompt()`、`classify_entries()`、`filter_for_llm()`、`merge_multipart_entries()` 等。所有提示词构建与条目筛选逻辑。外部词典提示注入。 |
| `src/llm/bridge.py` (319 行) | `LLMBridge` 类 — `review_batch()`（异步批处理审校）、`filter_verdicts()`（Phase 4 过滤）。`parse_review_response()` — 响应解析（4层容错）。`interactive_entry_review()` — 交互模式。 |
| `src/dictionary/external.py` (125 行) | `ExternalDictStore` — 按需 SQLite 查询社区词典，`lookup()` 按英文单词匹配历史翻译并注入 LLM 提示词。 |

所有 LLM 功能从 `src.llm` 包导入（`__init__.py` 统一重导出）。

### 3. `src/tools/key_alignment.py` — 键对齐 & 原版碰撞检测
...
# 原版 key 碰撞检测（从 Minecraft.db SQLite 读取）
collisions = check_vanilla_collisions(en_data, "data/Minecraft.db")
# → [{key, mod_value, vanilla_zh, version_start, version_end, changes}, ...]
```

**碰撞检测** 从 `data/Minecraft.db` 读取原版 key 及其版本区间（含 `vanilla_keys` 表的 `key/zh_cn/version_start/version_end/changes` 字段），模组 key 命中即报警。已从 `vanilla_keys.json` 迁移到 SQLite 支持的带版本区间格式。
...
| 唱片名   | `_check_music_disc_no_translation` | 须符合 `音乐唱片 - 曲名` 格式              |
| 占位符   | `_check_placeholder_integrity` | `%d/%s/%f`, `%n$s`, `%msg%`, `{0}` 个数一致       |
| 特殊标签 | `_check_special_tags`    | `§` 颜色码、`$(action)`、HTML、`<br>`、`\n` 数量一致 |
| tellraw  | `_check_tellraw_json`    | 仅翻译 `"text"` 字段，其余保留原文                   |
| 标点     | `_check_punctuation`     | 中英间距（可配置白名单）、标点规范                   |
| 尾空格   | `_check_trailing_whitespace` | 中文译文尾部有多余空格时提示                      |
| 能量单位 | `_check_energy_units`    | FE/RF/MB 等保留原文                                  |
| 省略号   | `_check_ellipsis`        | 中英省略号（`...`/`…`）一致性                       |
| 声音字幕 | `_check_sound_subtitle_format` | `主体：声音` 格式                               |
...
| `GITHUB_TOKEN`           | 否   | -                          | GitHub Token，PR 模式避免 60 req/hr 限流 |
...
JSON/Lang文件
   ...
   report_generator ────────── meta 表
        │
        ▼
   report.md / report.json / <ns>_report.md
```
...
**术语表 LLM 校验**：程序提取术语后，`llm_verify_glossary()` 取每条术语的 1 最长 + 4 最短含术语原文作为上下文，交 LLM 复核修正译文。仅当术语出现在 ≥2 个不同条目时触发校验；仅输出需要修正的术语，不通过的不输出。

**PR 模式术语源**：术语从 PR 内完整文件（`pr_full_en_data` / `pr_full_zh_data`）提取，而非仅从 diff 变更条目提取，确保术语统计更全面准确。
...
加载 `filtered=1` verdict，PASS 计入统计但不列入问题清单。按 namespace 分组输出 `report.json`、`report.md`、`<ns>_report.md`。Console 输出摘要 + 表格（前 30 行）。
...
GitHub Actions (`.github/workflows/test.yml`): 每次 push/PR 在 Python 3.11/3.12/3.13 三个版本运行 `pytest` + `pyright` 类型检查。

### 脚本

| 脚本 | 说明 |
|------|------|
| `scripts/download_external_dict.py` | 从 i18n-Dict-Extender 仓库下载 `Dict-Sqlite.db` |
| `scripts/migrate_minecraft_db.py` | 将 `Minecraft.db` 从 `(key, version)` 格式迁移为带版本区间的新格式 |
...
2. 方法签名：`def _check_xxx(self, key: str, en: str, zh: str) -> dict | None`
3. 在 `check_all()` 方法的 `checks` 列表中加入该函数
4. 如需配置项，添加到 `review_config.json` 和 `config.py`
...
本项目由 AI (DeepSeek-V4-Pro) 辅助生成，整体架构设计合理（Pipeline + DB 为单一真相源），但存在以下已知问题，修改时需注意。

### Critical — 功能影响

- **`shutil.rmtree()` 每次运行清空 output 目录** (`src/pipeline/pipeline.py:67`): 设计如此，但会把 `pipeline.db` 也删掉。勿在 `-o` 目录放其他文件。（不会修复）
- **3 处 Silent `except Exception: pass`**: 
  - `src/checkers/terminology_builder.py:170` — LLM 归并失败静默吞错
  - `src/checkers/terminology_builder.py:341` — LLM 术语校验失败静默吞错
  - `src/dictionary/external.py:26` — 停用词加载失败静默吞错
  排查 LLM 相关 bug 时需注意这些地方的异常被完全忽略。
- **`build_glossary()` 长达 117 行** (`src/checkers/terminology_builder.py:179-295`): 包含术语筛选、共识计算、中文互斥、短术语救援，应拆分为 3-4 个子函数。
- **`run_pr_aligner()` 长达 179 行** (`src/tools/pr/__init__.py:12-190`): God function，编排了整个 PR 对齐全流程（HTTP拉取、文件分组、JSON对齐、GuideME对齐、过滤、统计、输出），应拆为 `_fetch`, `_align_json`, `_align_guideme`, `_filter`, `_write` 等函数。

### High — 代码质量

- **`review_batch()` vs `review_untranslated()` 约 80% 重复** (`src/llm/bridge.py:104-214`): 两个方法的 `_process` 闭包几乎一样，仅 prompt 构建不同，应提取公共逻辑。
- **pyright 配置了大量 `=false`** (`pyrightconfig.json`): `reportArgumentType`, `reportAssignmentType`, `reportReturnType`, `reportAttributeAccessIssue`, `reportOptionalMemberAccess` 等核心检查均关闭。TypedDict 形同虚设，全局大量 `dict[str, Any]` 绕过类型系统。
- **LLM 模块无独立测试**: `bridge.py`(319行)、`prompts.py`(334行)、`client.py`(105行) 共 858 行零覆盖，仅在集成测试间接触及。
- **硬编码值散落 10+ 处**: 
  - `client.py:59` `MAX_RETRIES = 5`
  - `client.py:76-77` `temperature=0.1`, `max_tokens=32768`
  - `bridge.py:130` `for attempt in (1, 2)` 重试上限
  - `phase3b_fuzzy.py:9` 模糊触发模式
  - `phase3c_review.py:110` `batch_size=25`
  - `lemma_merge.py:98` `keys[:20]`
  - `terminology_extract.py:132` `keys[:5]`
  - `format_checker.py:142` `en[:60]`
  - `fuzzy_search.py:112` FTS recall 上限
  应收入 `review_config.json`。

### Medium — 代码结构

- **VERDICT_PRIORITY 重复定义**: `src/models.py:187` 和 `src/reporting/report_generator.py:35` 各定义一份，修改时需同步。
- **`PipelineDB` 无 context manager**: 不支持 `with` 语法，各 Phase 频繁 open/close，连接生命周期管理不一致。
- **`is_likely_code_or_proper_noun()` 逻辑重复**: `src/checkers/format_checker.py:77` 和 `src/tools/key_alignment.py:42` 有几乎相同的正则检测。
- **`apply_cache_merge()` 和 `apply_llm_merge()` 约 70% 重复** (`src/checkers/lemma_merge.py:55-104` vs `200-224`): 共享 merge map 应用逻辑。
- **`_check_placeholder_integrity()` 3 次重复比较逻辑** (`src/checkers/format_checker.py:162-197`): printf / `%msg%` / `{0}` 三种占位符使用相同 sorted/missing/extra 模式。

### Low — 风格/一致性

- **print() 与 sys.stderr 混用**: 无统一日志框架，进度信息输出目标不一致。
- **模块级全局单例**: `src/tools/fuzzy_search.py` 的 `_db_instance`、`src/config.py` 的 `_cfg_cache`，非线程安全但单进程够用。
- **`import time as _time` 放在 `main()` 函数体内** (`run.py:78`): 刻意为之，避免无关路径加载，但风格异常。
- **`TerminologyBuilder` 类承担过多职责**: 术语提取 + 词形归并 + 表构建 + LLM校验 + 一致性检查，约 363 行 10 方法。

### 未测模块（无独立测试文件）

| 模块 | 行数 | 备注 |
|------|------|------|
| `src/llm/bridge.py` | 319 | 仅在集成测试间接触及 |
| `src/llm/prompts.py` | 334 | 仅在集成测试间接触及 |
| `src/llm/client.py` | 105 | 仅在集成测试间接触及 |
| `src/dictionary/external.py` | 125 | 仅在集成测试间接触及 |
| `src/pipeline/phase1_alignment.py` | 97 | 仅在集成测试间接触及 |
| `src/pipeline/phase2_terminology.py` | 27 | 仅在集成测试间接触及 |
| `src/pipeline/phase3a_format.py` | 30 | 仅在集成测试间接触及 |
| `src/pipeline/phase3b_fuzzy.py` | 31 | 仅在集成测试间接触及 |
| `src/pipeline/phase3c_review.py` | 111 | 仅在集成测试间接触及 |
| `src/pipeline/phase4_filter.py` | 84 | 有独立测试 (33 行) |
| `src/pipeline/phase5_report.py` | 164 | 无独立测试 |
| `src/cli.py` | 93 | 无独立测试 |
| `src/tools/pr/__init__.py` | 162 | 无独立测试（仅 `_lang`/`_guideme` 有） |
