...
- **最终过滤** — Phase 5 再经 LLM 剔除误报，减少噪音；过滤结果持久化缓存（`filter_cache` 表），反复运行跳过已缓存条目
...
| `filter_cache` | Phase 5 过滤缓存（可持久复用） |
| `meta` | 统计信息与元数据 |
...
│   │   ├── phase4_report.py       # Phase 4 报告生成（写入 DB）
│   │   ├── phase5_filter.py       # Phase 5 最终 LLM 过滤（读/写 DB）
